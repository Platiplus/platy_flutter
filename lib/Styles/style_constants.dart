import 'package:flutter/material.dart';
// ------ COLORS
// UI
final appBackgroundColor = Color(0xFF223254);
final gradientBegin = Color(0xFF1680FF);
final gradientEnd = Color(0xFF7644AD);
final dividerColor = Color(0xFFD4D4D4);
final linearProgressForegroundColor = Color(0xFF4A9CFF);
final linearProgressBackgroundColor = Color(0xFFF2F2F2);
final circularProgressIndicatorColor = Color(0xFF6C4BB6);
final inputBorderColor = Color(0xFFDCDFE6);
final transactionStatusBorderColor = Color(0xFFD4D4D4);
final transactionPendingStatusBackgroundColor = Color(0xFFFFFFFF);
final transactionPaidStatusBackgroundColor = Color(0xFF1ABC9C);

// MENU
final loginMenuBackgroundColorUnselected = Color(0x20000000);
final loginMenuBackgroundColorSelected = Color(0xFFFFFFFF);
final controlMenuSelectedBarItemTextColor = Color(0xFFFFFFFF);
final controlMenuDeselectedBarItemTextColor = Color(0xFF879395);
final controlMenuLightBGBackgroundColor = Color(0xFFF2F2F2);

// ICON
final inputIconColor = Color(0xFFDCDFE6);
final inputSelectableIconColor = Color(0xFF6C4BB6);

// ------ FONTS
final primaryFontFamily = 'Montserrat';

// COLORS
final hintTextColor = Color(0xFF606A7B);
final dirtyInputTextColor = Color(0xFF333942);
final errorFeedbackColor = Color(0xFFED3C56);
final linkLightBGTextColor = Color(0xFF6C4BB6);
final linkDarkBGTextColor = Color(0xFFFFFFFF);
final buttonTextColor = Color(0xFFFFFFFF);
final moneyBalanceTextColor = Color(0xFF333942);
final supportTextColor = Color(0xFF606A7B);
final monthButtonTextColor = Color(0xFF333942);
final moneyValueTextColor = Color(0xFF333942);
final behaviorDescriptionTextColor = Color(0xFF606A7B);

// ------ SIZES
// UI
final moneyBalanceFontSize = 24.0;
final moneyValueFontSize = 14.0;
final linkFontSize = 12.0;
final supportFontSize = 12.0;
final behaviorDescriptionFontSize = 14.0;

// MENU
final sliderMenuFontSize = 12.0;
final monthButtonFontSize = 16.0;

// INPUT
final inputFontSize = 14.0;
final errorFeedbackFontSize = 12.0;

// ICON
final inputIconSize = 24.0;

// BUTTONS
final buttonLetterSpacing = 1.0;
final buttonFontSize = 16.0;
final snackFontSize = 16.0;

// MISC
final dirtyInputFontWeight = FontWeight.w500;
final moneyBalanceFontWeight = FontWeight.bold;

// COMPONENTS RELATED
final formFieldsStyle = BoxDecoration(
    color: Colors.white,
    borderRadius: BorderRadius.circular(50.0),
    border: Border.all(width: 1.0, color: inputBorderColor)
);